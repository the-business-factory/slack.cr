...
- Section now serializes a supplied `block_id`. A supplied `fields` collection
  must contain one to ten text objects. Text-only, fields-only, and combined
  sections remain valid.
- Actions now requires an `elements` argument and rejects nil or empty input.
  Its supported range is one to 25 buttons, instead of one to five.
- Modal title, submit, and close labels now accept at most 24 characters. Submit
  ...
  block is present. Close is optional. The existing six-argument modal helper
  remains available, and a named overload permits omitted labels.
...
valid constructor and six-argument modal-helper calls remain valid. Calls that
used nil or empty Actions elements, an empty Section fields array, labels longer
than 24 characters, invalid length ranges, or unnamed enums now fail locally.
...
- `crystal spec`: 283 examples, zero failures, zero errors, and zero pending.
